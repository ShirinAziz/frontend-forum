...
import jsImage from "./js.webp"; // Bild som används i startsidans hero-sektion
...
const HomePage = () => {
  ...
  useEffect(() => {
    // Sorterar trådarna efter skapelsedatum för senaste trådar
    const sortedByDate = [...threadsData].sort(
      (a, b) => new Date(b.created_at) - new Date(a.created_at)
    );
    setLatestThreads(sortedByDate.slice(0, 3));

    // Sorterar trådarna efter antal visningar för populära trådar
    const sortedByPopularity = [...threadsData].sort(
      (a, b) => b.views - a.views
    );
    setPopularThreads(sortedByPopularity.slice(0, 3));
  }, []);

  // Hanterar förändring av sökfältet
  ...
  // Hanterar klick på en specifik tråd
        ...
        <div className="hero-content flex-col lg:flex-row-reverse">
          ...
          <div>
            <h1 className="text-5xl font-bold">
              Välkommen till vårt utvecklarforum!
            </h1>
            <p className="py-6">
              Här kan du som programmerare diskutera kod, dela erfarenheter,
              lösa problem och få hjälp med tekniska utmaningar. Oavsett om du
              är nybörjare eller erfaren utvecklare, är du välkommen att bidra
              med dina kunskaper eller ställa frågor.
            </p>
